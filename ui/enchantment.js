...
export function renderEnchantmentUI() {
    ...
    const currentBonusHp = Math.round(baseStats.hp * enchantmentData.bonusHpPercent);
    const currentBonusDmg = (baseStats.baseDamage * enchantmentData.bonusDamagePercent).toFixed(1);
    
    let powerUpStatName = '';
    let currentPowerUpBonusValue = 0;
    let basePowerUpValue = 0;
    
    if (selectedBallType === 'explosive') {
        powerUpStatName = 'Explosion Radius';
        currentPowerUpBonusValue = enchantmentData.bonusPowerUpValue;
        basePowerUpValue = baseStats.radiusTiles;
    }
    
    const currentProductionCost = Math.round(HOME_BASE_PRODUCTION.BALL_COST_FOOD * enchantmentData.productionCostMultiplier);

    const outcomes = ENCHANTMENT_OUTCOMES[selectedBallType];
    // These are hardcoded based on balancing.js to avoid parsing function bodies
    const potentialHpBonus = Math.round(baseStats.hp * 0.15);
    const potentialDmgBonus = (baseStats.baseDamage * 0.20).toFixed(1);
    const potentialRadiusBonus = 0.2;

    const statsHTML = `
        <h3>${selectedBallType.charAt(0).toUpperCase() + selectedBallType.slice(1)} Ball - Level ${currentLevel} &rarr; ${currentLevel + 1}</h3>
        <div class="enchant-stats-grid">
            <span>Hit Point:</span>
            <span>${baseStats.hp} (<span class="bonus-value">+${currentBonusHp}</span>) <span class="arrow">&rarr;</span> <span class="potential-outcome">+${potentialHpBonus} ?</span></span>

            <span>Direct Damage:</span>
            <span>${baseStats.baseDamage} (<span class="bonus-value">+${currentBonusDmg}</span>) <span class="arrow">&rarr;</span> <span class="potential-outcome">+${potentialDmgBonus} ?</span></span>

            <span>${powerUpStatName}:</span>
            <span>${basePowerUpValue} (<span class="bonus-value">+${currentPowerUpBonusValue.toFixed(1)}</span>) <span class="arrow">&rarr;</span> <span class="potential-outcome">+${potentialRadiusBonus} ?</span></span>

            <span>Production Cost:</span>
            <span>🥕 ${currentProductionCost} <span class="arrow">&rarr;</span> <span class="potential-outcome">+?</span></span>
        </div>
    `;
    ...
    if (currentLevel < maxLevel) {
        ...
        const successRateDisplay = successRate.toFixed(0);
        ...
        controlsContainer.innerHTML = `
            <div>
                <strong>Success Rate: <span class="success-rate-color">${successRateDisplay}%</span></strong>
                <div class="progress-bar-container">
                    <div class="progress-bar-fill" style="width: ${successRateDisplay}%;"></div>
                </div>
            </div>
        `;
